...
@Component({
  selector: 'app-edit-mc',
  templateUrl: './edit-mc.page.html',
  styleUrls: ['./edit-mc.page.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EditMcPage implements OnInit {
  ...
  constructor(private _fb: FormBuilder, private _setupapiservuce: SetupApiService,
    ...
    this.unsubscribe$.sink = this._route.queryParams.subscribe(params => {
      ...
      if (this._router.getCurrentNavigation().extras.state) {
        this.xdata = this._router.getCurrentNavigation().extras.state.mc;
      }
    });
  }
  ...
  doSubmit(): void {
    ...
    this.unsubscribe$.sink = this._setupapiservuce.editMc(this.submitForm.value).subscribe(data => {
      ...
      if (this.apiresponse.message === 'ERROR') {
        ...
        this._loadingservice.routeAfter(600, `/app/settings/list-mc/${this._authservice.center.id}`, 'MC Edited Successfylly', 'middle', false, '');
      }
